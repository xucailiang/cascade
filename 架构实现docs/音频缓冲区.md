...
class AudioRingBuffer:
    def __init__(self, capacity_seconds, sample_rate, channels=1, dtype=np.float32):
        # ...
        self._buffer_lock = threading.RLock()  # 单一缓冲区锁
        self._event_not_empty = threading.Event()  # 非空事件
        self._event_not_full = threading.Event()  # 非满事件
        self._event_not_full.set()  # 初始状态为非满
```

- **单一锁设计**：使用单一的可重入锁保护整个缓冲区，简化锁管理
- **事件通知**：使用事件对象替代条件变量，提供更简单的异步通知机制
- **减少锁竞争**：通过简化锁结构减少锁竞争和死锁风险
...
def get_chunk_with_overlap(self, chunk_size, overlap_size):
    ...
    with self._read_lock:
        ...
        class ChunkMetadata(BaseModel):
            ...
            start_frame: int = Field(description="起始帧位置")
            chunk_size: int = Field(description="主要块大小（样本数）")
            overlap_size: int = Field(description="重叠区域大小（样本数）")
...
class AudioRingBuffer:
    def __init__(self, capacity_seconds, sample_rate, channels=1,
                 dtype=np.float32, strategy=BufferStrategy.BLOCK):
        # ...
        self.strategy = strategy  # 默认策略
        ...
        with self._buffer_lock:
            if self.available_data.get() + data_size > self.capacity:
                ...
                if self.strategy == BufferStrategy.BLOCK:
                    ...
                    # 拒绝新数据
                    raise BufferFullError("Buffer full, data rejected")
            
            # 写入数据...
```

- **策略枚举**：使用枚举定义有限的策略选项，提高代码可读性
- **初始化配置**：在初始化时设置默认策略，而不是每次调用时指定
- **统一接口**：提供单一的写入接口，根据配置的策略处理溢出情况
...
class ChunkMetadata(BaseModel):
    ...
    overlap_size: int = Field(description="重叠区域大小（样本数）")
    is_last: bool = Field(default=False, description="是否为最后一块")
    timestamp_ms: Optional[float] = Field(default=None, description="时间戳（毫秒）")
...
class AudioRingBuffer:
    ...
    def write(self, data: np.ndarray, timeout: Optional[float] = None) -> int:
        """写入数据到缓冲区，使用配置的默认策略"""
        pass
        
    def read(self, size: int, timeout: Optional[float] = None) -> np.ndarray:
        """从缓冲区读取指定大小的数据"""
        pass
    ...
    def get_chunk_with_overlap(self, chunk_size: int, overlap_size: int) -> Tuple[np.ndarray, ChunkMetadata]:
