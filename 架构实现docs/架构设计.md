...
本项目旨在解决音频流进入VAD（Voice Activity Detection，语音活动检测）之前的处理问题。当前VAD处理音频的方式存在严重延迟问题，每次VAD处理的音频块大小为256个采样点（约16ms），如果每次传入的音频大小是2s的音频，就需要将2s的音频切割为多个16ms的音频块交给VAD处理，这会导致严重的处理延迟。

### 1.2 核心需求

1. **并行处理**：将2s的音频切割为多块（根据线程数量而定），使用多个VAD实例并行处理
2. **顺序保证**：这些音频块有序列顺序，不管是哪个块检测到语音，最终都应该得到完整的正在讲话的语音块的事件戳
3. **异步处理**：整个处理过程和代码都要是异步的，使用uvicorn
...
9. **多后端支持**：对于VAD的启动和实例管理，要支持vllm和onnxruntime两种运行方式，使用工厂+继承的方式
